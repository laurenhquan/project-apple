<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        $query = sprintf("SELECT * FROM user WHERE user_name = '%s'", $mysqli->real_escape_string($_POST["user_name"])); // query to check if user exists in db
        ...
        if($existing_user) { // if user exists
            ...
            $query = "INSERT INTO user (user_name, pass_hash) VALUES (?, ?)"; // query to insert new user into db

            $stmt = $mysqli->stmt_init(); // create statement

            if(!$stmt->prepare($query)) { die("MySQL error: " . $mysqli->error); } // kill if error with preparing query
